...
from client.theme import hint_font, screen, ui_font
...
async def pygame_loop(websocket):
    ...
    while True:
        ...
        pygame.draw.rect(screen, (32, 32, 32), (sb_x, 0, SIDEBAR_W, h))
        pygame.draw.line(screen, (55, 55, 55), (sb_x, 0), (sb_x, h))

        header = ui_font.render("Live tuning", True, (130, 130, 130))
        screen.blit(header, (sb_x + SIDEBAR_PAD, 6))
        ...
        if sfx_hdr_y + 12 > HEADER_H and sfx_hdr_y < h:
            screen.blit(ui_font.render("SFX detection", True, (130, 130, 130)), (sb_x + SIDEBAR_PAD, sfx_hdr_y))
        ...
        for i, name in enumerate(BTN_LABELS):
            ...
            txt = ui_font.render(label, True, (240, 240, 240))
            screen.blit(txt, (rect.centerx - txt.get_width() // 2, rect.centery - txt.get_height() // 2))

        text_w = w - SIDEBAR_W - 36
        cap_top = 12 + TIP_H
        cap_bottom = h - FOOTER_H
        tip_surf = hint_font.render(fit_hint(compute_tuning_tip(), text_w), True, (150, 175, 210))
        screen.blit(tip_surf, (18, 14))
        ...
        for surf, y in get_caption_lines():
            ...
            if cap_top - LINE_H < dy < cap_bottom:
                screen.blit(surf, (18, dy))
        screen.set_clip(None)

        footer_y = h - FOOTER_H + 6
        sfx_max_w = w - SIDEBAR_W - 36
        ...
        if state["caption_telemetry"]:
            draw_noise_meter(screen, 34, h - 34)
